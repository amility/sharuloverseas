            ...
            <tbody>
               <tr>
                  <td align="center">
                     ...
                     <h5 style="font-weight: 500; font-size: 18px;">Your order has been received</h5>
         ...
         <table style="width: 100%; max-width: 700px; margin: -25px auto; padding: 25px 10px; text-align: center; color: #3d464d; margin: 0 auto; line-height: 10px;">
            ...
            {{$date->format('F d, Y g:i A')}}</p>
                  ...
                  <td style="border-right: 1px dashed #e7e7e7; width: 25%;">
                     ...
                     <p style="font-weight: 600; font-size: 1.1vw;">$ {{number_format($order['order']['total_price'], 2, '.', ',')}}</p>
                  </td>
                  <td style=" width: 25%;">
            ...
            <tbody>
              ...
              @endphp
               <tr>
                  ...
                  <td align="right" style="border-bottom: 2px solid #eee; padding: 0px 10px; font-size: 1.2vw;">$ {{$orderData['product_price']}}</td>
               </tr>
                @endforeach
            </tbody>
         </table>
         <table style="width: 100%; max-width: 700px; margin: 0px auto; padding: 25px 0px  0px 0px; text-align: center; color: #3d464d; line-height: 10px; border:3px solid #eee; border-top: none; " cellspacing="0" cellpadding="0">
            <tbody>
               <tr>
                ...
                @endif
                $ {{number_format($order['order']['total_price']-$order['order']['shipping_price']+$discount, 2, '.', ',')}}</td>
               </tr>
               <tr>
                  <td align="left" style="line-height: 35px; padding: 0px 10px; font-weight: 600;" >Shipping</td>
                  <td align="right" style="line-height: 35px; padding: 0px 10px; font-weight: 600;">$ {{$order['order']['shipping_price']}}</td>
               </tr>
               @if($order['order']['discount_price'])
               <tr>
                  <td align="left" style="line-height: 35px; padding: 0px 10px; font-weight: 600; border-bottom: 2px solid #eee;">Discount(-)</td>
                  <td align="right" style="line-height: 35px; padding: 0px 10px; font-weight: 600; border-bottom: 2px solid #eee;">$ {{number_format($order['order']['discount_price'], 2, '.', ',')}}</td>
               </tr>
               @endif
               <tr>
                  <td align="left" style="line-height: 35px; padding: 10px 10px; font-size: 28px; font-weight: 600;">Total</td>
                  <td align="right" style="line-height: 35px; padding: 10px 10px; font-size: 28px; font-weight: 600;">$ {{number_format($order['order']['total_price'], 2, '.', ',')}}</td>
               </tr>
            </tbody>
         </table>
                     ...
                     <div class="lst" style="display: flex;
                        ...
                        @endif
                            <b>Block : </b> {{ $order['shippingDetail']['block'] }}, <b>House & Building : </b> {{ $order['shippingDetail']['house_building'] }}, <b>Street : </b> {{ $order['shippingDetail']['street_address'] }} <br>
                             <b>Area & Country : </b> {{ getCityName($order['shippingDetail']['town_city']) .", ". getStateName($order['shippingDetail']['state_country']) ." ". $order['shippingDetail']['postcode_zip'] }}
                           </p>
                        ...
                        @endif
                            ...
                            <b>Area & Country : </b> {{ getCityName($order['billingDetail']['town_city']) .", ". getStateName($order['billingDetail']['state_country']) ." ". $order['billingDetail']['postcode_zip'] }}
                           </p>
                           <p><span style="font-size: 12px;">Phone Number<br></span>
