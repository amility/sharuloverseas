...
@section('content')
    @php
        ...
        if (!empty(Auth::guard('customer')->user())) {
            $customerData = Auth::guard('customer')
                ->user()
                ->toArray();
        } else {
            $customerData['id'] = 0;
    ...
    <!-- new Code start -->
    <div
        class="single-product woocommerce woocommerce-page body_transparent article_style_stretch scheme_original top_panel_show top_panel_above sidebar_hide bg_image shop-d-page">
        <!-- Breadcrumbs -->
                            ...
                            <div class="product">
                                ...
                                <div class="woocommerce-product-gallery woocommerce-product-gallery--with-images woocommerce-product-gallery--columns-4 images"
                                    ...
                                    <figure class="woocommerce-product-gallery__wrapper">
                                        <div data-thumb="{{ count($productDetails->images) > 0 ? asset($productDetails->images[0]->image) : asset('images/no-image.jpg') }}"
                                            class="woocommerce-product-gallery__image">
                                            @if (count($productDetails->images) == 0)
                                                <div class="product-image product-image--location--gallery">
                                                    <a href="{{ count($productDetails->images) > 0 ? asset($productDetails->images[0]->image) : asset('images/no-image.jpg') }}"
                                                        data-width="700" data-height="700" class="product-image__body"
                                                        target="_blank">
                                                        <img class="product-image__img"
                                                            src="{{ count($productDetails->images) > 0 ? asset($productDetails->images[0]->image) : asset('images/no-image.jpg') }}"
                                                            alt=""></a>
                                                </div>
                                            @else
                                                @foreach ($productDetails->images as $productImageLists)
                                                    <div class="product-image product-image--location--gallery">
                                                        <a href="{{ asset($productImageLists->image) }}" data-width="700"
                                                            data-height="700" class="product-image__body" target="_blank">
                                                            <img class="product-image__img"
                                                                src="{{ asset($productImageLists->image) }}" alt="">
                                                        </a>
                                                    </div>
                                                @endforeach
                                            @endif
                                ...
                                <div class="summary entry-summary">
                                    ...
                                    @if ($cart_product == null)
                                        ...
                                        <form class="product__options" action="{{ route('addToCarts.store') }}"
                                            method="post">
                                        @else
                                            @php $label = 'Update to cart'; @endphp
                                            <form class="product__options"
                                                action="{{ route('addToCarts.update', $cart_product['id']) }}"
                                                method="post">
                                                @method('PUT')
                                    @endif
                                    @csrf
                                    ...
                                    <input type="hidden" name="image"
                                        value="{{ count($productDetails->images) > 0 ? asset($productDetails->images[0]->image) : null }}">
                                    <div class="form-group product__option">
                                        ...
                                        @if($productDetails->seller_id>0)
                                        <br/>
                                        <label class="product__option-label"
                                            for="product-quantity"><b>Note: <span>Shipping might be delayed for 2-3 days</b></label>
                                            <br/>
                                        @endif    
                                        <label class="product__option-label text-danger"
                                            for="product-quantity">Quantity</label>
                                        <div class="product__actions">
                                            <div class="product__actions-item">
                                                <div class="input-number product__quantity">
                                                    <input name="quantity" id="product-quantity"
                                                        class="input-number__input my-2" type="number" min="1"
                                                        value="{{ $cart_product == null ? '1' : $cart_product['quantity'] }}"
                                            ...
                                            <div class="product__actions-item product__actions-item--addtocart">
                                                <button type="submit" class="btn btn-primary btn-lg" data-product_name=""
                                                    data-product_price="" data-quantity="1" data-image=""
                                                    @if ($productDetails->total_stock == 0) disabled @endif>{{ $label }}</button>
                                            </div>
                                ...
                                <div class="woocommerce-tabs wc-tabs-wrapper">
                                    ...
                                    <!-- Tab: Description -->
                                    <div class="woocommerce-Tabs-panel woocommerce-Tabs-panel--description panelreplace wc-tab"
                                        id="tab-description">
                                                    ...
                                                    <div class="post_item_wrap">
                                                        <div class="post_featured">
                                                            <div class="post_thumb">
                                                                <a
                                                                    href="{{ URL::to('/product', $featureProduct['id']) }}">
                                                                    <img src="{{ $featureProduct['images'] ? $featureProduct['images'][0]['image'] : asset('images/no-image.jpg') }}"
                                                                        alt="" title="Product" />
                                                                </a>
                                                            </div>
                                                        </div>
                                                        <div class="post_content">
                                                            <h2 class="woocommerce-loop-product__title">
                                                                <a
                                                                    href="{{ URL::to('/product', $featureProduct['id']) }}">{{ $featureProduct['prod_name'] }}</a>
                                                            </h2>
                                                            ...
                                                            <span class="price">
                                                                <span class="woocommerce-Price-amount amount">
                                                                    ...
                                                                    @if ($featureProduct['prod_price'] == $featureProduct['mrp_price'])
                                                                        <span>{{ CurrencySymbol() }}
                                                                            {{ $featureProduct['prod_price'] }}</span>
                                                                    @else
                                                                        <span>{{ CurrencySymbol() }}{{ $featureProduct['prod_price'] }}</span>
                                                                        &nbsp;<strike>{{ CurrencySymbol() }}{{ $featureProduct['mrp_price'] }}</strike>
                                                                        &nbsp;<span>
                                                                ...
                                                                @else
                                                                    <a class=" button add_to_cart_button  product-card__addtocart"
                                                                        href="{{ URL::to('customer/addToCarts') }}">Go to
                                                                        cart</a>
                                                                @endif
                                                                @if ($cart_product == null)
                                                                    <button
                                                                        class="button add_to_cart_button  product-card__addtocart product-card__addtocart--list"
                                                                        type="button"
                                                                        data-product_name="{{ $featureProduct['prod_name'] }}"
                                                                        data-product_price="{{ $featureProduct['prod_price'] }}"
                                                                        data-quantity="1"
                                                                        data-image="{{ $featureProduct['images'] ? $featureProduct['images'][0]['image'] : null }}"
                                                                        onclick="addtocart(this,{{ $featureProduct['id'] }})">Add
                                                                        To Cart</button>
                                                                @else
                                                                    <a class=" button add_to_cart_button btn btn-primary product-card__addtocart product-card__addtocart--list"
                                                                        href="{{ URL::to('customer/addToCarts') }}">Go to
                                                                        cart</a>
                                                                @endif
